...
- type: custom-api
  ...
  template: |
    ...
    {{ if gt (len $request_list) 0 }}
      ...
      {{ range $request_list }}
        {{/* Individual request list item card - card + widget-content-frame handle theme-consistent background/border/radius */}}
        <li class="card widget-content-frame gap-10">
          {{/* Link directly to the media item in Overseerr/Jellyseerr using its type and TMDB ID */}}
          <a href="http://<YOUR_SERVER_IP>:5055/{{ .String "type" }}/{{ .Int "tmdbId" }}" target="_blank" class="flex items-center gap-10" style="align-items: center;">
            {{ $poster := .String "poster" }}
            {{ if ne $poster "" }}
              {{/* Display poster image if available */}}
              <img class="card-seerr-requests" src="https://image.tmdb.org/t/p/w300{{ $poster }}" style="border-radius: var(--border-radius); min-width: 6rem; max-width: 6rem; aspect-ratio: 2/3; align-self: stretch; object-fit: cover; background: color-mix(in srgb, var(--color-text-base) 8%, var(--background-color));" loading="lazy">
            {{ else }}
              {{/* Fallback placeholder when poster is missing */}}
              <div class="flex items-center justify-center text-center" style="border-radius: var(--border-radius); min-width: 6rem; max-width: 6rem; aspect-ratio: 2/3; align-self: stretch; background: color-mix(in srgb, currentColor 10%, transparent); padding: 8px;">
